...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.business_message is not None and update.business_message.text is not None:
        ...
        if verify_permissions(update.business_message.chat_id, "chat"):
            ...
            await context.bot.send_message(chat_id=GetAdmins()[0], text="The current chat is not initialized. Please add the following ID to your list of approved IDs `" + str(update.business_message.chat_id) + "`",parse_mode="MarkdownV2" )
            return
